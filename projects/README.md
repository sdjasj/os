...
- 除 Ray 明确列出的使用示例外，不复制任何项目源码；Ray 示例也只限 `ray_usage_guide/examples/` 中的 Python 文件和 README。Codex、Hands-On Modern RL 与 Megatron-LM 教程同样不复制源码、用户配置、认证信息、模型、checkpoint、数据、日志、缓存、虚拟环境或构建产物。
