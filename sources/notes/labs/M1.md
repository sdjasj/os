...
<span class="float-left text-4xl mr-3 mt-2">⏰</span><span class="font-serif text-lg border-b border-slate-600">**Soft Deadline: 2026 年 4 月 5 日 23:59:59**</span>

**你需要首先阅读[实验须知](README.md)，其中包含了代码获取方法、提交方法、如何查看提交结果等**。在命令行中 `git pull origin M1` 下载框架代码。**你可以不使用框架代码中定义的函数、不使用单元测试——最终结果以 Online Judge 为准。**

<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**代码更新**</span>

3 月 22 日 12:30 更新了 testkit。在此之前下载的同学请和我们的 main 分支保持同步。

<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**学术诚信和 AIGC Policy**</span>

请确保你阅读了[实验须知](README.md)中的 AIGC Policy。

<span class="float-left text-4xl mr-3 mt-2">⚖️</span><span class="font-serif text-lg border-b border-slate-600">**M1 - labyrinth**</span>
...
![](../../img/cursor-tab.webp) (Cursor Tab 真懂我 😂)

## 2\. 实验描述

<span class="float-left text-4xl mr-3 mt-2">🗒️</span><span class="font-serif text-lg border-b border-slate-600">**实验要求：实现 labyrinth 命令行迷宫游戏**</span>
...
<span class="float-left text-4xl mr-3 mt-2">☕️</span><span class="font-serif text-lg border-b border-slate-600">**对上面的描述感到困惑？**</span>
...
1.  中括号扩起的参数是可选参数。因此 `-m`, `-p`, `--move`, `--version` 都是可选的参数。
2.  同一个选项可以有别名。在 `labyrinth` 中，`-m` 和 `--map` 的含义是一样的，`-p` 和 `--player` 的含义是一样的。

<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**请遵守 UNIX 对返回值的约定**</span>

`main` 函数的返回值代表了命令执行的状态，其中 `EXIT_SUCCESS` 表示命令执行成功，`EXIT_FAILURE` 表示执行失败。对于 POSIX 来说，0 代表成功，非 0 代表失败。和以前的 Online Judge 编程不同，实际的系统必须**在输入不符合规约时及时处理**。
