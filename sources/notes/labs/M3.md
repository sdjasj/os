...
<span class="float-left text-4xl mr-3 mt-2">⏰</span><span class="font-serif text-lg border-b border-slate-600">**Soft Deadline: 2026 年 4 月 19 日 23:59:59**</span>

在命令行中 `git pull origin M3` 下载框架代码。请注意[实验须知](README.md)中的 AIGC Policy。

<span class="float-left text-4xl mr-3 mt-2">⚖️</span><span class="font-serif text-lg border-b border-slate-600">**M3 - sperf**</span>
...
<span class="float-left text-4xl mr-3 mt-2">🗒️</span><span class="font-serif text-lg border-b border-slate-600">**实验要求：实现系统调用 Profiler**</span>

实现一个命令行工具，它能启动另一个程序，并统计该程序中各个系统调用的占用时间。对于较短时间运行的程序，你可以一次性打印出耗时最多的几个系统调用。对于耗时较长的程序，你需要保证每秒大约**打印 10 次** (不要过多) 系统调用的耗时信息。

### 2.1 总览

sperf <span class="underline">COMMAND</span> \[<span class="underline">ARG</span>\]...
...
<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**只能使用 execve**</span>
...
<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**及时清空 stdout 的缓冲区**</span>
...
<span class="float-left text-4xl mr-3 mt-2">☕️</span><span class="font-serif text-lg border-b border-slate-600">**实现 strace**</span>
...
而系统调用是一个程序与操作系统交互的**唯一**方式 (这个说法并不严格；还可以通过共享内存实现与操作系统内核的通信)。strace 从 execve “重置程序” 的系统调用开始，列出进程和操作系统的一切交互。所以从本质上讲，我们只需要**解析 strace 的输出**就能完成这个实验。

<span class="float-left text-4xl mr-3 mt-2">☕️</span><span class="font-serif text-lg border-b border-slate-600">**阅读系统调用序列**</span>
...
<span class="float-left text-4xl mr-3 mt-2">💡</span><span class="font-serif text-lg border-b border-slate-600">**计算机系统公理**</span>
...
<span class="float-left text-4xl mr-3 mt-2">☕️</span><span class="font-serif text-lg border-b border-slate-600">**写出正确的 sperf**</span>
