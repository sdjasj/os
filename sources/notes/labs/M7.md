...
<span class="float-left text-4xl mr-3 mt-2">⏰</span><span class="font-serif text-lg border-b border-slate-600">**Soft Deadline: 2026 年 5 月 31 日 23:59:59**</span>

在命令行中 `git pull origin M7` 下载框架代码。请注意[实验须知](README.md)中的 AIGC Policy。

<span class="float-left text-4xl mr-3 mt-2">⚖️</span><span class="font-serif text-lg border-b border-slate-600">**M7 - httpd**</span>

请输入 Token 登录。

## 1\. 背景

HTTP (Hypertext Transfer Protocol) 是互联网上最广泛使用的应用层协议之一。从简单的静态网页 (例如《操作系统》的课程网站) 到复杂的 Web 应用 (例如我们的教务系统到能在网页中直接使用的 Office 365)，它们都是由 HTTP 服务器支撑的！

HTTP “服务器” 的原理比大家想象得简单，它是一个文本协议，从客户端接受一个文本请求，然后返回一个 response。例如以下是一个典型的 HTTP 请求：
...
httpd \[<span class="underline">port</span>\]
...
3.  按照请求到来的顺序输出日志到标准输出，在每次日志输出后都请确保执行 fflush。我们已经提供了 log\_request 函数，请调用它。

<span class="float-left text-4xl mr-3 mt-2">⚠️</span><span class="font-serif text-lg border-b border-slate-600">**log\_request**</span>
...
这个实验对大语言模型来说是 “刚好适合” 的任务，甚至我们的参考实现都是由 LLM 直接一次生成正确的。但为了获得最佳的学习效果，你可以借助大语言模型查阅 CGI 的相关标准，询问任何不理解的地方，但请你不要使用大语言模型直接实现代码。
...
你可以在浏览器中查看完整的 HTTP Request 和 Response，甚至你可以直接让 codex 操纵你的浏览器完成这件事！现在，“understanding” 变得前所未有的重要。

## 3\. 正确性标准

首先，你需要正确地实现 CGI 脚本的执行。我们会在 cgi-bin 目录下放置若干 CGI 脚本，并使用 curl 命令测试你的 httpd 是否能够正确地执行这些脚本。如果你的 httpd 能正确解析 URL、传递环境变量 REQUEST\_METHOD 和 QUERY\_STRING，并正确返回 CGI 脚本的输出，可以通过 Easy Test Cases。
...
函数的参数是一个 “client\_socket”，它实际上是一个文件描述符 (更精确地说，是一个 socket 描述符)，当然我们也可以使用 read 和 write 读写——只是我们用了操作系统提供的网络 API 中的 send 和 recv，你把它们理解成读取连接到 “远端” 管道的 read 和 write 即可。它首先读出一个不太长的 http request，然后构造一个 HTTP/1.1 200 OK 的 response，回复的字符串是 “Under construction”。
