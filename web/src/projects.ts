...
const projectSeeds: ProjectSeed[] = [
  ...
  },
  {
    ...
    mark: 'PI',
    title: 'Pi',
    subtitle: '小内核终端编程代理源码学习',
    description: '沿 CLI、模型流、Agent 循环、工具、TUI、扩展、会话与远程协议，建立 TypeScript Coding Agent 的完整心智模型。',
    category: 'AI Agent',
    level: '入门 → 进阶',
    tags: ['Coding Agent', 'TypeScript', 'TUI', 'Extensions'],
    tone: 'blue',
    updatedAt: '2026-08-28',
    snapshot: '4e49492',
    repositoryUrl: 'https://github.com/earendil-works/pi',
    repositoryRef: '4e494929998d6bc4fccf75e0a233f727db4b70ee',
    tracks: [{
      id: 'source', eyebrow: 'SOURCE WALKTHROUGH', title: '源码与实践主线',
      description: '先补足异步流、Schema 与终端背景，再沿真实调用链读通模型、Agent、产品编排、扩展和持久化。',
      copiedDirectory: 'pi/guide/', originalDirectory: '',
      include: (path) => /^\d{2}-.+\.md$/.test(path),
      originalPath: (path) => ({
        '00-study-guide.md': 'README.md',
        '01-background.md': 'packages/ai/src/types.ts',
        '02-architecture.md': 'package.json',
        '03-development.md': 'AGENTS.md',
        '04-ai-layer.md': 'packages/ai/src/models.ts',
        '05-agent-loop.md': 'packages/agent/src/agent-loop.ts',
        '06-tui.md': 'packages/tui/src/tui-main-screen.ts',
        '07-coding-agent.md': 'packages/coding-agent/src/core/agent-session.ts',
        '08-tools.md': 'packages/coding-agent/src/core/tools/edit.ts',
        '09-resources-extensions.md': 'packages/coding-agent/src/core/resource-loader.ts',
        '10-sessions-compaction.md': 'packages/coding-agent/src/core/session-manager.ts',
        '11-observability-storage.md': 'packages/telemetry/src/index.ts',
        '12-sdk-rpc-protocol.md': 'packages/protocol/src/framing.ts',
        '13-testing-debugging.md': 'packages/agent/test/agent-loop.test.ts',
        '14-learning-projects.md': 'packages/coding-agent/examples/extensions/hello.ts',
        '15-source-index-glossary.md': 'README.md',
      } as Record<string, string>)[path] ?? 'README.md',
    }],
  },
  {
  ...
  },
  {
    ...
    mark: 'VL',
    title: 'verl',
    subtitle: '从 GRPO 主链到多阶段 Agentic RL',
    description: '沿真实源码读通 Ray/FSDP 训练、SGLang rollout、Agent Loop 与 Reward Loop，并以 CyberGym-E2E 演示复杂环境的多阶段奖励接入。',
    category: 'AI 与训练系统',
    level: '入门 → 进阶',
    tags: ['GRPO', 'SGLang', 'Agentic RL', 'Reward Engineering'],
    tone: 'violet',
    updatedAt: '2026-07-27',
    snapshot: '983cb0f',
    repositoryUrl: 'https://github.com/verl-project/verl',
    repositoryRef: '983cb0f24443f87b3d161fad318445130a620b07',
    repositoryRoots: [{
      pathPrefix: 'cybergym-e2e-upstream/',
      url: 'https://github.com/sunblaze-ucb/cybergym-e2e',
      ref: 'b861317f11641b14ab6ba08b5179d0b044601057',
    }],
    safetyNote: 'CyberGym-E2E 仅作为授权隔离环境的训练系统案例；教程不导入漏洞样本、PoC 或补丁。',
    tracks: [{
      id: 'source', eyebrow: 'SYSTEM TO PRACTICE', title: '源码、训练与环境接入主线',
      description: '先补齐 GRPO 与分布式训练背景，再沿 verl 真实数据流读到 SGLang、多轮工具和 reward，最后完成端到端实验。',
      copiedDirectory: 'verl/guide/', originalDirectory: '',
      include: (path) => /^\d{2}-.+\.md$/.test(path),
      originalPath: (path) => ({
        '00-learning-roadmap.md': 'examples/grpo_trainer/README.md',
        '01-rl-grpo-foundations.md': 'verl/trainer/ppo/core_algos.py',
        '02-verl-architecture.md': 'verl/trainer/ppo/ray_trainer.py',
        '03-config-data-contracts.md': 'verl/utils/dataset/rl_dataset.py',
        '04-sglang-rollout.md': 'verl/workers/rollout/sglang_rollout/async_sglang_server.py',
        '05-agent-loop-tools.md': 'verl/experimental/agent_loop/tool_agent_loop.py',
        '06-reward-loop.md': 'verl/experimental/reward_loop/reward_loop.py',
        '07-cybergym-e2e-model.md': 'cybergym-e2e-upstream/scripts/validate.py',
        '08-cybergym-bridge-tool.md': 'cybergym-e2e-upstream/scripts/utils.py',
        '09-dataset-and-tool-config.md': 'examples/data_preprocess/gsm8k_tool_agent_loop.py',
        '10-multistage-reward.md': 'verl/experimental/reward_loop/reward_manager/naive.py',
        '11-training-launch.md': 'examples/grpo_trainer/run_qwen3_8b_fsdp.sh',
        '12-debugging-capstone.md': 'verl/trainer/ppo/ray_trainer.py',
      } as Record<string, string>)[path] ?? 'README.md',
    }],
  },
  {
    ...
    mark: 'DS',
    title: 'DeepSpeed',
    subtitle: '从训练 Engine 到 ZeRO 与组合并行',
    description: '沿一次训练 step 贯通配置、通信、混合精度、ZeRO、Offload、Pipeline、TP/SP、MoE、Checkpoint、推理与 DeepCompile。',
    category: 'AI 与训练系统',
    level: '入门 → 进阶',
    tags: ['ZeRO', 'Distributed Training', 'Parallelism', 'Inference'],
    tone: 'blue',
    updatedAt: '2026-08-27',
    snapshot: '32e301f',
    repositoryUrl: 'https://github.com/deepspeedai/DeepSpeed',
    repositoryRef: '32e301ffaf5a7b2ae74be7a25b22c917bb4bf05a',
    tracks: [
      {
        id: 'foundations', eyebrow: 'START HERE', title: '基础与训练运行时',
        description: '补齐分布式与显存背景，再沿初始化、配置和 Engine 生命周期建立全局主线。',
        copiedDirectory: 'deepspeed/foundations/', originalDirectory: '',
        include: (path) => /^0[0-4]-.+\.md$/.test(path),
        originalPath: (path) => ({
          '00-learning-roadmap.md': 'README.md',
          '01-distributed-and-memory-foundations.md': 'deepspeed/runtime/utils.py',
          '02-repository-architecture-and-installation.md': 'setup.py',
          '03-initialize-and-configuration.md': 'deepspeed/__init__.py',
          '04-engine-training-lifecycle.md': 'deepspeed/runtime/engine.py',
        } as Record<string, string>)[path] ?? 'README.md',
      },
      {
        id: 'zero', eyebrow: 'MEMORY SYSTEMS', title: 'ZeRO 与内存系统',
        description: '从混合精度和集合通信出发，推导 ZeRO 三阶段、参数生命周期与 CPU/NVMe Offload。',
        copiedDirectory: 'deepspeed/zero/', originalDirectory: '',
        include: (path) => /^(?:0[5-9]|10)-.+\.md$/.test(path),
        originalPath: (path) => ({
          '05-mixed-precision-and-optimizers.md': 'deepspeed/runtime/fp16/loss_scaler.py',
          '06-communication-layer-and-collectives.md': 'deepspeed/comm/comm.py',
          '07-zero-principles-and-memory-model.md': 'deepspeed/runtime/zero/config.py',
          '08-zero-stage1-stage2.md': 'deepspeed/runtime/zero/stage_1_and_2.py',
          '09-zero-stage3-parameter-lifecycle.md': 'deepspeed/runtime/zero/stage3.py',
          '10-offload-cpu-nvme.md': 'deepspeed/runtime/zero/offload_config.py',
        } as Record<string, string>)[path] ?? 'README.md',
      },
      {
        id: 'parallel', eyebrow: 'SCALE OUT', title: '组合并行与推理',
        description: '读通 Pipeline、张量/序列并行、MoE、Checkpoint 和推理引擎之间的边界与组合方式。',
        copiedDirectory: 'deepspeed/parallel/', originalDirectory: '',
        include: (path) => /^1[1-5]-.+\.md$/.test(path),
        originalPath: (path) => ({
          '11-pipeline-parallelism.md': 'deepspeed/runtime/pipe/schedule.py',
          '12-tensor-and-sequence-parallelism.md': 'deepspeed/sequence/layer.py',
          '13-moe-and-expert-parallel.md': 'deepspeed/moe/layer.py',
          '14-checkpointing-and-recovery.md': 'deepspeed/runtime/checkpoint_engine/checkpoint_engine.py',
          '15-inference-engine.md': 'deepspeed/inference/engine.py',
        } as Record<string, string>)[path] ?? 'README.md',
      },
      {
        id: 'engineering', eyebrow: 'ENGINEERING', title: '启动、测试与编译扩展',
        description: '把多节点启动、硬件抽象、算子构建、测试调试、性能分析与 DeepCompile 串成工程实践。',
        copiedDirectory: 'deepspeed/engineering/', originalDirectory: '',
        include: (path) => /^1[6-9]-.+\.md$/.test(path),
        originalPath: (path) => ({
          '16-launcher-and-multinode.md': 'deepspeed/launcher/runner.py',
          '17-accelerator-and-op-builder.md': 'accelerator/real_accelerator.py',
          '18-testing-debugging-and-profiling.md': 'tests/unit/common.py',
          '19-deepcompile-capstone-and-glossary.md': 'deepspeed/compile/backend.py',
        } as Record<string, string>)[path] ?? 'README.md',
      },
    ],
  },
  {
    ...
    mark: 'MCore',
    title: 'Megatron-LM',
    subtitle: '从 GPT 数学到多维并行训练系统',
    description: '沿一次 GPT 预训练迭代，贯通数据、模型规格、Attention、MoE、TP/PP/CP/DP、优化器、Checkpoint、测试与集群实践。',
    category: 'AI 与训练系统',
    level: '入门 → 进阶',
    tags: ['LLM Training', 'Tensor Parallel', 'Pipeline Parallel', 'MoE'],
    tone: 'violet',
    updatedAt: '2026-08-27',
    snapshot: 'e79cb4c',
    repositoryUrl: 'https://github.com/NVIDIA/Megatron-LM',
    repositoryRef: 'e79cb4c1bae1afd04322d979d08cb63832991ebe',
    tracks: [
      {
        id: 'foundations', eyebrow: 'FOUNDATIONS', title: '背景与系统基础',
        description: '先补齐 GPT 张量、PyTorch 分布式、GPU 性能与多维并行拓扑，再进入真实训练源码。',
        copiedDirectory: 'megatron-lm/foundations/', originalDirectory: '',
        include: (path) => /^\d{2}-.+\.md$/.test(path),
        originalPath: (path) => ({
          '00-project-map.md': 'README.md',
          '01-gpt-math-and-shapes.md': 'megatron/core/datasets/gpt_dataset.py',
          '02-distributed-pytorch.md': 'megatron/core/parallel_state.py',
          '03-gpu-performance-and-precision.md': 'megatron/core/model_parallel_config.py',
          '04-parallelism-topology.md': 'docs/user-guide/parallelism-guide.md',
        } as Record<string, string>)[path] ?? 'README.md',
      },
      {
        id: 'source', eyebrow: 'SOURCE WALKTHROUGH', title: 'GPT 预训练源码主线',
        description: '从启动参数和进程组开始，逐层读通数据、模型、并行、训练循环与可恢复 Checkpoint。',
        copiedDirectory: 'megatron-lm/source/', originalDirectory: '',
        include: (path) => /^\d{2}-.+\.md$/.test(path),
        originalPath: (path) => ({
          '00-entrypoint-to-pretrain.md': 'pretrain_gpt.py',
          '01-arguments-and-config.md': 'megatron/training/arguments.py',
          '02-initialization-and-groups.md': 'megatron/training/initialize.py',
          '03-data-pipeline.md': 'megatron/core/datasets/gpt_dataset.py',
          '04-model-construction.md': 'gpt_builders.py',
          '05-module-spec-and-layer.md': 'megatron/core/transformer/transformer_layer.py',
          '06-attention-internals.md': 'megatron/core/transformer/attention.py',
          '07-mlp-and-moe.md': 'megatron/core/transformer/moe/moe_layer.py',
          '08-tensor-and-sequence-parallel.md': 'megatron/core/tensor_parallel/layers.py',
          '09-pipeline-schedules.md': 'megatron/core/pipeline_parallel/schedules.py',
          '10-context-parallel-and-packing.md': 'megatron/core/utils.py',
          '11-data-parallel-and-optimizer.md': 'megatron/core/distributed/distributed_data_parallel.py',
          '12-training-loop.md': 'megatron/training/training.py',
          '13-checkpoint-and-recovery.md': 'megatron/training/checkpointing.py',
        } as Record<string, string>)[path] ?? 'README.md',
      },
      {
        id: 'labs', eyebrow: 'HANDS-ON', title: '运行、测试与集群实践',
        description: '在 dev 容器中完成最小训练、rank/shape 推演、测试、性能诊断和 SLURM 多节点练习。',
        copiedDirectory: 'megatron-lm/labs/', originalDirectory: '',
        include: (path) => /^\d{2}-.+\.md$/.test(path),
        originalPath: (path) => ({
          '00-environment-and-first-run.md': 'examples/run_simple_mcore_train_loop.py',
          '01-rank-shape-trace.md': 'megatron/core/parallel_state.py',
          '02-testing-system.md': 'tests/unit_tests/models/test_gpt_model.py',
          '03-performance-and-debugging.md': 'megatron/training/training.py',
          '04-slurm-and-contribution.md': 'docs/developer/contribute.md',
        } as Record<string, string>)[path] ?? 'README.md',
      },
    ],
  },
